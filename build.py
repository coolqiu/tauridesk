...
def main():
    ...
    if windows:
        ...
        if pa:
            ...
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        target_exe_name = f'rustdesk-{version}-win7-install.exe'
        shutil.copy2('target/release/RustDesk.exe', os.path.join(res_dir, target_exe_name))
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python ./generate.py -f ../../{res_dir} -o . -e {target_exe_name}')
        shutil.move(f'../../{res_dir}/{target_exe_name}', '../..')
